...
import { CheckCircle2, Clock, AlertTriangle, User, ChevronRight, ImageIcon } from "lucide-react";
...
export default function DiagnosisTimeline({
  ...
  const sortedDiagnoses = [...diagnoses].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

  const getStatusIcon = (diagnosis: Diagnosis) => {
    switch (diagnosis.status) {
      case "completed": return <CheckCircle2 className="h-5 w-5 text-emerald-500" />;
      case "pending_review": return <Clock className="h-5 w-5 text-amber-500" />;
      case "reviewed": return <User className="h-5 w-5 text-sky-500" />;
  ...
  const getStatusText = (diagnosis: Diagnosis) => {
    switch (diagnosis.status) {
      ...
      case "pending_review": return "Pending Doctor Review";
      case "reviewed": return "Doctor Reviewed";
    ...
    <div className="space-y-4">
      {sortedDiagnoses.map((diagnosis) => (
        <button
          ...
          className={`w-full text-left transition-colors ${selectedDiagnosisId === diagnosis._id ? "bg-sky-50 dark:bg-sky-900/20" :
              "bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700"
            } rounded-xl p-4 shadow-sm border ${selectedDiagnosisId === diagnosis._id ? "border-sky-200 dark:border-sky-800" :
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                <span
                  className={
                    diagnosis.status === "completed" ? "text-emerald-700 dark:text-emerald-400" :
                      diagnosis.status === "pending_review" ? "text-amber-700 dark:text-amber-400" :
                        diagnosis.status === "reviewed" ? "text-sky-700 dark:text-sky-400" :
                          diagnosis.status === "not_reviewed" ? "text-red-700 dark:text-red-400" :
  ...
  {diagnosis.isEczema}
              ...
              <div className="mt-1 text-xs text-slate-500 dark:text-slate-400 flex items-center gap-2">
                ...
                {diagnosis.recommendations && diagnosis.recommendations.length > 0 && (
                  <><span className="text-slate-400 dark:text-slate-500">•</span><span>{diagnosis.recommendations.length} recommendations</span></>
                )}
              </div>
            </div>
